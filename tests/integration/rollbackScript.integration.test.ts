...
describe('rollback-script integration', () => {
  ...
  beforeEach(() => {
    ...
    global.fetch = vi.fn(async (url: string, _opts?: any) => {
      ...
      if (typeof url === 'string' && url.includes('/script_versions') && _opts?.method !== 'POST') {
      ...
      if (_opts?.method === 'PATCH' && url.includes('/scripts')) {
        posts.push({ patch: true, body: JSON.parse(_opts.body) });
